...
def filter_targets(nations, my_nation, min_infra=1500, max_infra=20000, 
    ...
    for n in nations:
        try:
            ...
            results.append({
                ...
                "alliance": "No Alliance" if n["alliance_id"] == "0" or n["alliance_id"] is not None else "Has Alliance",
    ...
    # Sort by money lost in last war
    return sorted(results, key=lambda x: (x["money_lost"], x["infra"]), reverse=True)[:5]
